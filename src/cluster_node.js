...
program
  ...
  .option('-d --define <key=value>', 'define (service) value', value => configStatements
    .push(value))
  ...
  .action((args, options, logger) => {
    ...
    Promise.all([kronosModules(), expand(options.config ? "${include('" + path.basename(options.config) + "')}" : {
      services: {
        registry: {
          // consul
          checkInterval: 60
